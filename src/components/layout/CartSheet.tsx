...
export function CartSheet() {
  ...
  return (
    <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
      <SheetContent
        side="right"
        className="w-[90vw] sm:max-w-md flex flex-col p-0 bg-white shadow-2xl border-l border-gray-100 z-[100]"
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50">
          {cartItems.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center gap-4 text-gray-500">
              ...
              <p className="text-lg font-body text-brand-dark-soft font-medium">
                Keranjangmu masih kosong.
              </p>
              <p className="text-sm font-body max-w-[250px]">
                Yuk pilih alat petualanganmu dulu di katalog kami!
              </p>
              <Button
                asChild
              ...
              {cartItems.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-brand-dark-soft text-sm md:text-base leading-tight truncate">
                      {item.name}
                    </h4>
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      <div className="flex items-center bg-gray-50 rounded-lg border border-gray-200">
                        <button
                          ...
                          className="p-1.5 hover:text-brand-orange text-gray-500 transition-colors"
                        >
                          <Minus size={14} />
                        </button>
        ...
        {cartItems.length > 0 && (
          <div className="p-6 border-t border-gray-100 bg-white shadow-[0_-10px_30px_rgba(0,0,0,0.02)] flex-shrink-0">
            <div className="flex justify-between items-center mb-4">
              <span className="text-gray-500 font-medium font-body">
                Total Harga
              </span>
              <span className="text-2xl font-bold font-display text-brand-dark-soft">
                Rp {total.toLocaleString("id-ID")}
              </span>
            </div>
            <Button
              size="lg"
              className="w-full bg-brand-dark-soft hover:bg-brand-dark text-white shadow-lg shadow-brand-dark/20 border-none"
              disabled={isLoading}
              onClick={handleCheckout}
            >
              {session ? "Lanjut ke Checkout" : "Masuk untuk Checkout"}
            </Button>
            <p className="mt-3 text-center text-xs text-gray-500">
              {session
                ? "Checkout diproses di halaman akun."
                : "Buat akun atau masuk dulu untuk melanjutkan checkout."}
            </p>
          </div>
        )}
      </SheetContent>
